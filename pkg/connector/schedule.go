...
func (s *scheduleResourceType) List(ctx context.Context, _ *v2.ResourceId, pt *pagination.Token) ([]*v2.Resource, string, annotations.Annotations, error) {
	...
	rosters, nextPageToken, annos, err := s.client.GetRosters(ctx, page)
	if err != nil {
		...
		if servicenow.IsInvalidTableError(err) {
			return nil, "", annos, errOnCallModuleAbsent(err)
		}
		if servicenow.IsAccessDeniedError(err) {
			return nil, "", annos, errOnCallAccessDenied(err)
		}
		return nil, "", annos, fmt.Errorf("baton-servicenow: failed to list schedules: %w", err)
	}
...
func (s *scheduleResourceType) Grants(ctx context.Context, resource *v2.Resource, pt *pagination.Token) ([]*v2.Grant, string, annotations.Annotations, error) {
	...
	// An empty incoming page token means this is the first page; only then do we
	// also emit the (unpaginated) on-call and manager grants.
	firstPage := pt.Token == ""

	rotaMembers, nextPageToken, annos, err := s.client.GetRotaMembers(
		ctx,
	...
	if err != nil {
		...
		return nil, "", annos, fmt.Errorf("baton-servicenow: failed to list schedule members: %w", err)
	}
	...
	for _, member := range mapRotaMembers(rotaMembers) {
		...
		if err != nil {
			return nil, "", annos, fmt.Errorf("baton-servicenow: error creating principal id: %w", err)
		}
	...
	if firstPage {
		...
		for _, oc := range onCall {
			...
			if err != nil {
				return nil, "", annos, fmt.Errorf("baton-servicenow: error creating on-call principal id: %w", err)
			}
			rv = append(rv, grant.NewGrant(resource, scheduleOnCall, rID))
		}

		// Reuse rota_id from the resource profile so manager resolution skips a
		// redundant GetRoster; fall back to a roster lookup if it's absent.
		rotaID := ""
		if resource.HasProfile() {
			rotaID, _ = rs.GetProfileStringValue(resource.GetProfile(), "rota_id")
		}
		managerID, err := s.scheduleManagerUserID(ctx, resource.Id.Resource, rotaID)
		if err != nil {
		...
		if managerID != "" {
			...
			if err != nil {
				return nil, "", annos, fmt.Errorf("baton-servicenow: error creating manager principal id: %w", err)
			}
...
func (s *scheduleResourceType) scheduleManagerUserID(ctx context.Context, rosterId string, rotaID string) (string, error) {
	if rotaID == "" {
	...
	rota, err := s.client.GetRota(ctx, rotaID)
	if err != nil {
		return "", fmt.Errorf("baton-servicenow: failed to get rota %s: %w", rotaID, err)
	}
	if rota.Group == "" {
		return "", nil
	}
	group, _, err := s.client.GetGroup(ctx, rota.Group)
	if err != nil {
...
func (s *scheduleResourceType) Grant(ctx context.Context, principal *v2.Resource, entitlement *v2.Entitlement) (annotations.Annotations, error) {
	...
	existing, _, _, err := s.client.GetRotaMembers(ctx, rosterId, principal.Id.Resource, servicenow.KeysetPaginationVars{Limit: 1})
	if err != nil {
	...
	if groupId != "" {
		inGroup, _, _, err := s.client.GetUserToGroup(ctx, principal.Id.Resource, groupId, servicenow.KeysetPaginationVars{Limit: 1})
		if err != nil {
			return nil, fmt.Errorf("baton-servicenow: failed to check assignment group membership: %w", err)
		}
		if len(inGroup) == 0 {
			if _, err := s.client.AddUserToGroup(ctx, servicenow.GroupMemberPayload{User: principal.Id.Resource, Group: groupId}); err != nil {
	...
	annos, err := s.client.AddOnCallMember(ctx, servicenow.OnCallAddMemberPayload{
...
func (s *scheduleResourceType) Revoke(ctx context.Context, grant *v2.Grant) (annotations.Annotations, error) {
	...
	annos, err := s.client.RemoveOnCallMember(ctx, servicenow.OnCallRemoveMemberPayload{
