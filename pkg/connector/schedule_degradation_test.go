...
func TestScheduleList_OnCallModuleAbsent_ErrorsClearly(t *testing.T) {
	...
	client, err := servicenow.NewClient(uhttp.NewBaseHttpClient(srv.Client()), "user:pass", "dev", nil, nil, nil, srv.URL)
	if err != nil {
...
func newClient(t *testing.T, srv *httptest.Server) *servicenow.Client {
	t.Helper()
	c, err := servicenow.NewClient(uhttp.NewBaseHttpClient(srv.Client()), "user:pass", "dev", nil, nil, nil, srv.URL)
	if err != nil {
		t.Fatalf("NewClient: %v", err)
	}
	return c
