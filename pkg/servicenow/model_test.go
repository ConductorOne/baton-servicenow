...
func TestPrepareRotaMemberFilter(t *testing.T) {
	tests := []struct {
		...
		{"roster only", "ros456", "", nil, "roster=ros456"},
		{"both", "ros456", "usr789", nil, "roster=ros456^member=usr789"},
		{"member only", "", "usr789", nil, "member=usr789"},
		{"neither", "", "", nil, ""},
		{"enumeration scoped to domains", "ros456", "", []string{"example.com"}, "roster=ros456^member.emailENDSWITH@example.com"},
		{"point lookup ignores domains", "ros456", "usr789", []string{"example.com"}, "roster=ros456^member=usr789"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := prepareRotaMemberFilter(tt.rosterId, tt.memberId, tt.domains)
