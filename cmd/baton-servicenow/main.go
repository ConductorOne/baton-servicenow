...
func getConnector(ctx context.Context, snc *config.ServiceNow) (types.ConnectorServer, error) {
	...
	servicenowConnector, err := connector.New(ctx, auth, snc.Deployment, ticketSchemaFilters, snc.AllowedDomains, snc.CustomUserFields, snc.BaseUrl, snc.Insecure, snc.Incremental, snc.StateDir)
	if err != nil {
